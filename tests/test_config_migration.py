...
from pandaemon.config.loader import load_config, save_config
...
def test_onboard_refresh_rewrites_legacy_config_template(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("pandaemon.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr("pandaemon.cli.commands.get_workspace_path", lambda: workspace)
