...
import pandaemon.channels.matrix as matrix_module
from pandaemon.bus.events import OutboundMessage
from pandaemon.bus.queue import MessageBus
from pandaemon.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from pandaemon.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("pandaemon.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "pandaemon.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("pandaemon.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "pandaemon.channels.matrix.asyncio.create_task", _fake_create_task
    )
